...
import 'pest_prediction_screen.dart';

class ImagePreviewScreen1 extends StatelessWidget {
  final String imagePath;

  const ImagePreviewScreen1({Key? key, required this.imagePath})
      : super(key: key);
  ...
  Future<void> _processImage(BuildContext context) async {
    ...
    // ✅ Upload image to API
    Map<String, dynamic>? result = await apiService.uploadPestImage(imageFile);

    Navigator.pop(context); // Close loading dialog

    if (result != null) {
      String pest = result["predicted_class"];
      double confidence = result["confidence_score"];
      ...
      await FirebaseFirestore.instance.collection('pestPredict').add({
        "userId": user?.uid,
      ...
      // ✅ Navigate to Pest Prediction Screen
      Navigator.push(
        context,
        MaterialPageRoute(
